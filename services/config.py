...
class ConfigStore:
    ...
    def sub2api_billing_dsn(self) -> str:
        return str(
            os.getenv("SUB2API_BILLING_DSN")
            or self.data.get("sub2api_billing_dsn")
            or ""
        ).strip()

    @property
    def sub2api_billing_allowed_group_names(self) -> list[str]:
        env_value = os.getenv("SUB2API_BILLING_ALLOWED_GROUP_NAMES")
        value = env_value if env_value is not None else self.data.get("sub2api_billing_allowed_group_names")
        if isinstance(value, str):
            items = value.split(",")
