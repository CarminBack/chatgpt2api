...
export function ImageAccessPolicyCard() {
  ...
  const setUserIdsText = useSettingsStore((state) => state.setImage1kOnlySub2APIUserIdsText);
  const setKeyIdsText = useSettingsStore((state) => state.setImage1kOnlySub2APIKeyIdsText);
      ...
      <CardContent className="space-y-5 p-6">
        <div className="flex flex-col gap-3 border-b border-stone-100 pb-5 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Badge variant="info">用户 {userIds.length}</Badge>
            <Badge variant="warning">密钥 {keyIds.length}</Badge>
          </div>
        </div>
